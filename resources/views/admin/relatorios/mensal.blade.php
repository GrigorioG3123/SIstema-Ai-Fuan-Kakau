...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Relatório Mensal - {{ $nomeMes[$mes] }} {{ $ano }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
        ...
        <div class="container-fluid">
            ...
            <div class="row mb-4">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <form method="GET" action="{{ route('admin.relatorio.mensal') }}" class="form-inline">
                                <label for="ano" class="mr-2">Ano:</label>
                                <select name="ano" id="ano" class="form-control mr-3" onchange="this.form.submit()">
                                    @for($year = date('Y') - 5; $year <= date('Y'); $year++)
                                        <option value="{{ $year }}" {{ $ano == $year ? 'selected' : '' }}>{{ $year }}</option>
                                    @endfor
                                </select>
                                <label for="mes" class="mr-2">Mês:</label>
                                <select name="mes" id="mes" class="form-control mr-3" onchange="this.form.submit()">
                                    @foreach($nomeMes as $key => $nome)
                                        <option value="{{ $key }}" {{ $mes == $key ? 'selected' : '' }}>{{ $nome }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="card report-card">
                        <div class="card-header report-header">
                            <div class="report-title">
                                <h2 class="mb-0">RELATÓRIO MENSAL DE CAFÉ</h2>
                                <h4 class="mb-0 text-muted">Sistema de Gestão CCT - Timor Leste</h4>
                            </div>
                            <div class="report-info">
                                <p class="mb-0"><strong>Mês/Ano:</strong> {{ $nomeMes[$mes] }} {{ $ano }}</p>
                        ...
                        <div class="card-body">
                            ...
                            <div class="report-section">
                                ...
                                <div class="row">
                                    @php
                                        $produsaun = $dadosMensais->where('tipo', 'produsaun')->first();
                                        $venda = $dadosMensais->where('tipo', 'venda')->first();
                                        $totalProdusaun = $statsMes['total_produsaun'] ?? 0;
                                        $totalVenda = $statsMes['total_venda'] ?? 0;
                                        $totalValor = $statsMes['total_revenue'] ?? 0;
                                    ...
                                    <div class="col-md-4">
                                        <div class="performance-summary">
                                            ...
                                            <div class="performance-item">
                                                ...
                                                <span class="performance-value">{{ $statsMes['total_transacoes_prod'] ?? 0 }}</span>
                                            </div>
                                            <div class="performance-item">
                                                <span class="performance-label">Transações Venda:</span>
                                                <span class="performance-value">{{ $statsMes['total_transacoes_venda'] ?? 0 }}</span>
                                            </div>
                                            <div class="performance-item">
                                    ...
                                    <table class="table table-bordered report-table">
                                        ...
                                        <tfoot class="table-footer">
                                            <tr>
                                                ...
                                                <td class="text-center"><strong>{{ ($statsMes['total_transacoes_prod'] ?? 0) + ($statsMes['total_transacoes_venda'] ?? 0) }}</strong></td>
                                                <td colspan="2"></td>
                                            </tr>
                                        </tfoot>
                                    </table>
                            ...
                            <!-- Daily Performance -->
                            @if($dadosDiarios->count() > 0)
                            <div class="report-section">
                                <h3 class="section-title">DESEMPENHO DIÁRIO</h3>
                                <div class="table-responsive">
                                    <table class="table table-bordered report-table">
                                        <thead class="table-header">
                                            <tr>
                                        ...
                                        <tbody>
                                            @foreach($dadosDiarios->sortBy('data') as $dado)
                                            <tr>
